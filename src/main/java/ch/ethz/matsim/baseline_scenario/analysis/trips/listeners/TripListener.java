...
public class TripListener implements ActivityStartEventHandler, ActivityEndEventHandler, PersonDepartureEventHandler, PersonEntersVehicleEventHandler,
        PersonLeavesVehicleEventHandler, LinkEnterEventHandler, TeleportationArrivalEventHandler {
    ...
    public TripListener(Network network, StageActivityTypes stageActivityTypes, HomeActivityTypes homeActivityTypes, MainModeIdentifier mainModeIdentifier,
        ...
        if (!event.getPersonId().toString().contains("av")) {
            if (!stageActivityTypes.isStageActivity(event.getActType())) {
                ...
                ongoing.put(event.getPersonId(),
                        new TripListenerItem(event.getPersonId(), personTripIndex, network.getLinks().get(event.getLinkId()).getCoord(), event.getTime(), event.getActType()));
    ...
    public void handleEvent(PersonDepartureEvent event) {
        // if (!event.getPersonId().toString().contains("av") && !event.getLegMode().toString().contains("av")) {
        if (!event.getPersonId().toString().contains("av")) {
    ...
    public void handleEvent(ActivityStartEvent event) {
        if (!event.getPersonId().toString().contains("av")) {
            if (stageActivityTypes.isStageActivity(event.getActType())) {
                ongoing.get(event.getPersonId()).elements.add(factory.createActivityFromLinkId(event.getActType(), event.getLinkId()));
            } else {
                ...
                if (trip != null) {
                    ...
                    trips.add(new TripItem(trip.personId, trip.personTripId, trip.origin, trip.destination, trip.startTime, trip.travelTime, trip.networkDistance, trip.mode,
    ...
    public void handleEvent(PersonEntersVehicleEvent event) {
        if (!event.getPersonId().toString().contains("av") && !event.getVehicleId().toString().contains("av")) {
